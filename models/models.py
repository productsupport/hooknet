...
def context_target_unet(context_input, target_input, nClass, n_conv=2, nfilter0=32, growth=2):
    ...
    # Block 4: context
    nfilter *= growth
    c_conv4_d = ConvBlock(n_conv, nfilter, c_pool3_d)
    c_conv4_d = ConvBlock(n_conv, nfilter, c_conv4_d)

    # Block 5 - up: context
    # nfilter /= growth
    # c_up5 = UpSampling2D(size=(2, 2), data_format=None, interpolation='nearest')(c_conv4_d)
    c_up5 = Conv2DTranspose(nfilter, kernel_size=(2, 2), strides=(2, 2))(c_conv4_d)
    c_merge5 = Concatenate()([c_conv3_d, c_up5])
    c_conv5_u = ConvBlock(n_conv, nfilter, c_merge5)

    # Block 6 - up: context
    nfilter /= growth
    # c_up6 = UpSampling2D(size=(2, 2), data_format=None, interpolation='nearest')(c_conv5_u)
    ...
    c_conv7_u_crop = Cropping2D(cropping=((112, 112)))(c_conv7_u)


    # Block 4: target
    t_conv4_d = ConvBlock(n_conv, nfilter, t_pool3_d)
    t_conv4_d_concat = Concatenate()([t_conv4_d, c_conv7_u_crop])
    t_conv4_d = ConvBlock(n_conv, nfilter, t_conv4_d_concat)

    # Block 5 - up: target
    # nfilter /= growth
    # t_up5 = UpSampling2D(size=(2, 2), data_format=None, interpolation='nearest')(t_conv4_d)
    t_up5 = Conv2DTranspose(nfilter, kernel_size=(2, 2), strides=(2, 2))(t_conv4_d)
