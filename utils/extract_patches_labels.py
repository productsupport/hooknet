...
def main(slidePath, annotationPath, outputFolder, counter, N, tsz=256, separate_files=True):  # outputPath
    # WSI
...
#                   'IDC', 'Stroma', 'Lymfocyten', 'Reactive changes', 'Adipose', 'RedBlood', 'Benigne', 'ILC']
    ...
    xytiles_anno, tileannoidx, tileannoidxlabel = utils.anno_coords(xytiles, annotation.coords, annotation.labelid, tsz)
...
#             outputPath = os.path.join(args.outfolder, sid + ".h5")
    ...
    Parallel(n_jobs=num_cores)(delayed(main)(path[0], path[1], path[2], counter, len(images)) for counter, path in enumerate(images))
